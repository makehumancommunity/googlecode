...
def compute_distance2(x, y=None):
    if y is None:
        gram = np.dot(x,x.T)
        diag = gram.diagonal()
        return diag[:,np.newaxis] + diag[np.newaxis] - 2 * gram
    else:
        gram = np.dot(x, y.T)
        diagx = (x*x).sum(-1)
        diagy = (y*y).sum(-1)
        return diagx[:,np.newaxis] + diagy[np.newaxis] - 2* gram


class CWarp2(object):

    def __init__(self, source, target, landmarks):
        self.source = np.asarray(source, dtype="float32")
        self.target = np.asarray(target, dtype="float32")

        self.xverts = self.source[landmarks]
        self.yverts = self.target[landmarks]
        H = self.rbf(self.xverts)
        w = np.linalg.lstsq(H,self.yverts)[0]
        self.w = w
    ...
    def rbf(self, x, y=None):
        ...
        if y is None:
            dmax = dists2.max()
            dtmp = dists2 + dmax * np.identity(x.shape[0])
            self.s2 = dtmp.min(0)

        return np.sqrt(dists2 + self.s2)
        #~ return np.exp(- 0.003 * dists2 / dists2.max())


    def warpTarget(self, morph):
        idx = morph.keys()
        disp = np.asarray(morph.values(), dtype="float")
        xmorph = self.source[idx] + disp
        H = self.rbf(xmorph, self.xverts)
        ymorph = np.dot(H, self.w) - self.target[idx]
...
class CWarp1:
    ...
    def setup(self, xverts, yverts):
        ...
        for k in range(3):
            self.w[k] = np.arange(n, dtype=float)
            for i in range(n):
                self.w[k][i] = 0.1

            self.y[k] = np.arange(n, dtype=float)
            for i in range(n):
        ...
        self.HTH = np.dot(self.HT, self.H)
    ...
    def solve(self, index):
        A = self.HTH
        b = np.dot(self.HT, self.y[index])
        self.w[index] = np.linalg.solve(A, b)
        #e = self.y[index] - np.dot(self.H, self.w[index])
    ...
    def warpTarget(self, morph, source, target, landmarks):
        xverts = [ np.array(source[n]) for n in landmarks]
        yverts = [ np.array(target[n]) for n in landmarks]

        self.setup(xverts, yverts)

        ymorph = {}
        for n in morph.keys():
            xloc = morph[n] + source[n]
...
def test_warp():
    ...
    morph = dict([ (i+n/2, 0.1 * np.random.rand(3)) for i in range(n/2) ])
    landmarks = range(n/2)
    ...
    log.message("difference morph1/morph2 %s", np.abs(np.array(ymorph.values()) - np.array(ymorph2.values())).mean() )
    log.message("morph error 1 %s", np.abs(np.array(morph.values()) * (1,3,1) - np.array(ymorph.values())).mean() )
    log.message("morph error 2 %s", np.abs(np.array(morph.values()) * (1,3,1) - np.array(ymorph2.values())).mean() )
